...
class TimerDict(Timer):
    def __init__(self, image_list, img_lists, start_index=0, delay=100, is_loop=True):
        super().__init__(image_list, start_index, delay, is_loop)
        self.timer_dict = {}
        # img_lists contains tuples ("name", [image_list]),...
        for name, img_list in img_lists:
            self.timer_dict[name] = img_list
    ...
    def switchTo(self, key):
        if self.hasKey(key):
            self.image_list = self.timer_dict[key]
            self.frames = len(self.image_list)
            self.index = self.start_index if self.start_index < self.frames - 1 else 0
